...
export default function OnboardingPage() {
  ...
  const handleSaveForLater = async () => {
    ...
    const response = await fetch('/api/onboarding/save-progress', {
      ...
      body: JSON.stringify({ ...formData, step, clerkId: user?.id, email: user?.emailAddresses[0]?.emailAddress }),
    })
  ...
  const handleSubmit = async () => {
    ...
    const response = await fetch('/api/onboarding', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...formData, riskScore, clerkId: user?.id, email: user?.emailAddresses[0]?.emailAddress }),
    })
    if (response.ok) router.push('/dashboard')
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: c.cream, fontFamily: sans, fontWeight: 300 }}>
      ...
      <header style={{
        ...
        padding: '1.25rem 3rem', borderBottom: `0.5px solid ${c.border}`,
        ...
        <button
          ...
          disabled={isSaving}
          style={{ fontFamily: mono, fontSize: '0.72rem', letterSpacing: '0.06em', textTransform: 'uppercase', color: c.inkMuted, background: 'none', border: 'none', cursor: 'pointer', opacity: isSaving ? 0.5 : 1 }}
        >
      ...
      <div style={{ borderBottom: `0.5px solid ${c.border}` }}>
        ...
        <div style={{ display: 'flex', padding: '0 3rem' }}>
          {stepTitles.map((title, i) => (
            <div key={title} style={{ flex: 1, padding: '1rem 0', borderRight: i < 3 ? `0.5px solid ${c.border}` : 'none' }}>
              <span style={{ fontFamily: mono, fontSize: '0.62rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: i + 1 === step ? c.green : i + 1 < step ? c.inkMuted : c.border }}>
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '4rem 3rem' }}>
        ...
        <h1 style={{ fontFamily: serif, fontSize: 'clamp(2rem, 4vw, 2.75rem)', fontWeight: 400, letterSpacing: '-0.03em', lineHeight: 1.1, marginBottom: '2.5rem', color: c.ink }}>
          {stepTitles[step - 1]}
        </h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
          ...
          {step === 1 && (
            <>
              <div>
                <label style={labelStyle}>Primary investment goal</label>
              ...
              <div>
                <label style={labelStyle}>When do you plan to use this money?</label>
          ...
          {step === 2 && (
            <>
              <div>
                <label style={labelStyle}>If your portfolio dropped 20%, you would...</label>
              ...
              <div>
                <label style={labelStyle}>Your attitude toward risk</label>
              ...
              <div>
                <label style={labelStyle}>Comfortable level of portfolio fluctuation</label>
          ...
          {step === 3 && (
            <>
              ...
              <div>
                <label style={labelStyle}>Emergency fund (3–6 months expenses)</label>
              ...
              <div>
                <label style={labelStyle}>% of income you can invest monthly</label>
              ...
              <div>
                <label style={labelStyle}>Starting paper trading balance</label>
          ...
          {step === 4 && (
            <>
              <div>
                <label style={labelStyle}>Investing experience</label>
              ...
              <div>
                <label style={labelStyle}>Investing mindset</label>
              ...
              <div>
                <label style={labelStyle}>Portfolio management preference</label>
              ...
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <button onClick={() => setStep(3)} style={{ flex: 1, padding: '0.9rem', fontFamily: mono, fontSize: '0.75rem', letterSpacing: '0.06em', textTransform: 'uppercase', background: 'white', color: c.inkSoft, border: `0.5px solid ${c.border}`, borderRadius: '2px', cursor: 'pointer' }}>
                  Back
                </button>
                <button
                  onClick={handleSubmit}
                  disabled={!formData.experienceLevel || !formData.investingMindset || !formData.managementPreference}
                  style={{ flex: 1, padding: '0.9rem', fontFamily: mono, fontSize: '0.75rem', letterSpacing: '0.06em', textTransform: 'uppercase', background: c.green, color: c.cream, border: 'none', borderRadius: '2px', cursor: 'pointer', opacity: (!formData.experienceLevel || !formData.investingMindset || !formData.managementPreference) ? 0.4 : 1 }}
                >
                  Complete setup
                </button>
              </div>
...
function StepButtons({ step, setStep, canContinue }: { step: number; setStep: (s: number) => void; canContinue: boolean }) {
  return (
    <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
      {step > 1 && (
        <button onClick={() => setStep(step - 1)} style={{ flex: 1, padding: '0.9rem', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', letterSpacing: '0.06em', textTransform: 'uppercase' as const, background: 'white', color: '#4a4a44', border: '0.5px solid #ddd9ce', borderRadius: '2px', cursor: 'pointer' }}>
          Back
        </button>
      )}
      <button
        ...
        style={{ flex: 1, padding: '0.9rem', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', letterSpacing: '0.06em', textTransform: 'uppercase' as const, background: '#1C3D2B', color: '#F5F2EB', border: 'none', borderRadius: '2px', cursor: canContinue ? 'pointer' : 'not-allowed', opacity: canContinue ? 1 : 0.4 }}
      >
        Continue
      </button>
    </div>
...
function calculateRiskScore(data: any): number {
  ...
  const hasExperience = data.previousInvestments?.length > 1 && !data.previousInvestments.includes('None')
